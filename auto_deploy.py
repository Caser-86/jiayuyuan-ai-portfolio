...
# === 配置 ===
HOST = os.environ.get("ECS_HOST", "121.41.169.43")
PORT = int(os.environ.get("ECS_PORT", "22"))
USER = os.environ.get("ECS_USER", "root")
PASSWORD = os.environ.get("ECS_PASSWORD")  # 必须从环境变量读取，禁止硬编码
LOCAL_DIR = Path(os.environ.get("LOCAL_DIR", r"d:\Files\jiayuyuan-ai-portfolio"))
REMOTE_DIR = os.environ.get("REMOTE_DIR", "/root/jiayuyuan")
SCRIPT_FILE = "deploy-aliyun.sh"
DOMAIN = os.environ.get("DOMAIN", "libaweiyu.xyz")
ADMIN_EMAIL = os.environ.get("ADMIN_EMAIL", "776706958@qq.com")

if not PASSWORD:
    print("错误：必须设置环境变量 ECS_PASSWORD")
    print("示例：export ECS_PASSWORD='your_password'")
    sys.exit(1)
