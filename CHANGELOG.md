...
- **Hero 区域后台可配置** — 首页数字统计、按钮、打字机文案全部可在管理后台调整
- **统计数字开关** — 可关闭整个统计区域，或单独隐藏某一项
- **按钮可开关** — 主按钮（查看作品集）和次按钮（下载简历）可独立显示/隐藏并自定义链接
- **打字机文案后台编辑** — 支持多个文案循环，用英文逗号分隔

### 🔧 后端

- `profile.json` 新增 `hero` 配置对象：`showStats`、`stats`、`showTyping`、`typingTexts`、`primaryButton`、`secondaryButton`
- `/api/profile` 接口透传并持久化 `hero` 配置
